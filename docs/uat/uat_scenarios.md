...
| UAT-ALB-011 | 登录审计表缺失回归 | 备份一份运行中数据库并手动 `DROP TABLE audit_events` | 1. 重启应用<br>2. 登录管理员账号 6 次：前 5 次输错密码触发限速，第 6 次输入正确<br>3. 观察登录请求响应与日志 | 登录错误返回 401，成功登录返回 302/200，无 500；错误日志输出“Missing audit_events”警告，但会话保持有效 | 验证 audit_events 缺失时的容错 |
